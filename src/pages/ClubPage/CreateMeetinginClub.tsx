...
const CreateMeeting: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    else if (title.length < 3 || title.length > 10) newErrors.title = "제목은 3~10글자 내로 작성해주세요.";

    if (!description) newErrors.description = "필수 입력입니다.";
    else if (description.length < 10 || description.length > 100) newErrors.description = "설명은 10~100글자 내로 작성해주세요.";

    if (!location) newErrors.location = "필수 입력입니다.";
    else if (location.length < 3 || location.length > 10) newErrors.location = "장소는 3~10글자 내로 작성해주세요.";
    ...
    const newMeeting = {
      ...
      description,
      location,
      memberLimit: typeof memberLimit === "string" ? parseInt(memberLimit) : memberLimit,
      date,
